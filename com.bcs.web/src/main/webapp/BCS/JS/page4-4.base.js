...
$(function(){
	$('.btn_add').click(function(){
 		window.location.replace(bcs.bcsContextPath +'/edit/msgCreatePage?from=msgListSchedulePage');
	});
	...
	var btn_copyFunc = function(){
		...
		console.info('btn_copyFunc msgId:' + msgId);
 		window.location.replace(bcs.bcsContextPath + '/edit/msgCreatePage?msgId=' + msgId + '&actionType=Copy&from=msgListSchedulePage');
	};
	...
	var loadDataFunc = function(){
		...
		$.ajax({
			type : "GET",
			url : bcs.bcsContextPath +'/edit/getSendMsgList?status=SCHEDULED&sendType=SCHEDULE'
		}).success(function(response){
			$('.dataTemplate').remove();
			...
			for(key in response.MsgMain){
				...
				msgData.find('.msgContent a').attr('href', bcs.bcsContextPath +'/edit/msgCreatePage?msgId=' + msgId + '&actionType=Look&from=msgListSchedulePage');
