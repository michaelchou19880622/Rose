...
import com.bcs.core.linepoint.bot.scheduler.handler.ExecuteSendAllRichMenuTask;
...
import com.bcs.core.linepoint.bot.scheduler.handler.ExecuteSendRichMenuTask;
...
import com.bcs.core.linepoint.db.entity.LinePointSendGroup;
import com.bcs.core.linepoint.db.service.LinePointSendGroupService;
import com.bcs.core.linepoint.web.ui.model.SendLinePointDetailModel;
import com.bcs.core.linepoint.web.ui.model.SendLinePointModel;
...
@Service
public class LinePointSendUIService {
	@Autowired
	private LinePointSendGroupService LinePointSendGroupService;
	@Autowired
...
//	private RichMenuContentService richMenuContentService;
	/** Logger */
	private static Logger logger = Logger.getLogger(LinePointSendUIService.class);
	
	// Send SendRichMenuModel
	@Transactional(rollbackFor=Exception.class, timeout = 30)
	public void sendMessage(SendLinePointModel sendRichMenuModel, String adminUserAccount) throws Exception{
		// check GroupId
	...
	public void sendByGroupId(Long groupId) throws Exception {
		...
		// check LinePointSendGroup
		LinePointSendGroup sendGroup = LinePointSendGroupService.findOne(groupId);
		if(sendGroup == null) {
		...
		// Handle : IMMEDIATE
		if(LinePointSendGroup.GROUP_TYPE_DEFAULT.equals(sendGroup.getGroupType())) {
			...
			if(sendGroup.getGroupId() > 0){
				...
				// 預設群祖
				Long result= LinePointSendGroupService.countDefaultGroupSize(sendGroup.getGroupId());
				if(result != null){
					if(result > 0){
						// 設定成功
					}else{
...
//	}
	...
	public void saveDraftMessage(SendLinePointModel SendRichMenuModel, String adminUserAccount) throws Exception{
	 ...
	 */
	@Transactional(rollbackFor=Exception.class, timeout = 30)
	private Long saveMessage(SendLinePointModel SendRichMenuModel, String adminUserAccount, String status) throws Exception{
		...
		List<SendLinePointDetailModel> list = SendRichMenuModel.getSendMsgDetails();
		this.createMsgDetail(msgMain.getMsgId(), list, adminUserAccount, MsgMain.THIS_PARENT_TYPE);
		
		return msgMain.getMsgId();
	}
	
	public void createMsgDetail(Long msgId, List<SendLinePointDetailModel> list, String adminUserAccount, String parentType ) throws Exception{

		if(list != null && list.size() > 0){
			for(SendLinePointDetailModel model : list){
	 ...
	 */
	@Transactional(rollbackFor=Exception.class, timeout = 30)
	public void redesignMsg(SendLinePointModel SendRichMenuModel, String adminUserAccount) throws Exception{ 
	...
	private List<Message> validateMessagesWichMessage(List<SendLinePointDetailModel> sendMsgDetails, String mid) throws Exception{
		return validateMessagesWichMessage(sendMsgDetails, mid, null);
	}
	
	private List<Message> validateMessagesWichMessage(List<SendLinePointDetailModel> sendMsgDetails, String mid, Map<String, String> replaceParam) throws Exception{

		List<Message> result = new ArrayList<Message>();
		for(SendLinePointDetailModel sendMsgDetail : sendMsgDetails){
	 ...
	 * @throws Exception
	 */
	private List<MsgGenerator> validateMessages(List<SendLinePointDetailModel> sendMsgDetails) throws Exception{
	...
	public void setGroups(Map<String, Object> result){
		...
		try{
			Map<Long, String> groups = LinePointSendGroupService.findGroupTitleMap();
