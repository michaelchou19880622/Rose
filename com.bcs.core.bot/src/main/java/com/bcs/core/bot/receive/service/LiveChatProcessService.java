...
public class LiveChatProcessService {
	...
	public void startProcess(String channelId, String replyToken, String UID, String category) throws Exception {
		try {
			...
			if (userLiveChat != null) {
				...
				case UserLiveChat.BEGINNING:
					...
					if (resultStatus == LiveChatStartResponse.WAITING) {
						...
						sender = switchIconService.generateSenderModel(CONFIG_STR.AUTO_REPLY.toString());
			...
			messageProcessService.replyMessageWithServiceCode(channelId, replyToken, messageList, CONFIG_STR.AUTO_REPLY.toString());
		} catch (Exception e) {
		...
		try {
			...
			if (userLiveChat != null) {
				...
				case UserLiveChat.IN_PROGRESS:
					isInprogress = true;
					sender = switchIconService.generateSenderModel(CONFIG_STR.MANUAL_REPLY.toString());
			...
			String channelName = isInprogress == true ? CONFIG_STR.MANUAL_REPLY.toString() : CONFIG_STR.AUTO_REPLY.toString();
	...
	public void handleSwitchAction(String type, String channelId, String UID, String replyToken) throws Exception {
		...
		switch (type) {
			...
			case "AutoReply":
				...
				messageProcessService.replyTextMessageWithServiceCode(channelId, CONFIG_STR.AUTO_REPLY.toString(), textList, replyToken);
				break;
	...
	public void handleWaitingAction(String waitingAction, String UID) throws Exception {
		...
		case "leaveMessage":
			...
			messageProcessService.pushMessageWithServiceCode(UID, messageList, CONFIG_STR.AUTO_REPLY.toString());
			break;
		case "giveUp":
			this.giveUpWaiting(UID);

			messageProcessService.pushTextMsgAsyncWithServiceCode(UID, LiveChatWordingUtil.getString(LIVE_CHAT_WORDING.GIVEUP_MESSAGE.toString()), CONFIG_STR.AUTO_REPLY.toString());
			break;
		}
	}
	...
	private void keepWaiting(String UID) throws Exception {
		...
		if (userLiveChat != null) {
			...
			if (result.getError())
				...
				messageProcessService.pushTextMsgAsyncWithServiceCode(UID, LiveChatWordingUtil.getString(LIVE_CHAT_WORDING.WAITING_MESSAGE.toString()), CONFIG_STR.AUTO_REPLY.toString());
		} else {
