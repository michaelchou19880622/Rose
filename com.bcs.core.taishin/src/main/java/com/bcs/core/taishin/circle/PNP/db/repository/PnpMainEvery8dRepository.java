...
//	@Modifying
//	@Query("update PnpMainEvery8d a set a.sendCount = a.sendCount +1 where a.pnpMainId = ?1")
//	@Transactional(rollbackFor=Exception.class, timeout = 30)
//	void increaseSendCountByPnpMainId(Long pnpMainId);
//	
//	@Modifying
//	@Query("update PnpMainEvery8d a set a.sendCount = a.sendCount +?2 where a.pnpMainId = ?1")
//	@Transactional(rollbackFor=Exception.class, timeout = 30)
//	void increaseSendCountByPnpMainId(Long pnpMainId, Long increase);
	
	@Transactional(readOnly = true, timeout = 30)
	public List<PnpMainEvery8d> findByStatus(String status);
	
//	@Query("select x from PnpMainEvery8d x where x.status in (?1) and x.tempId in (?2) ")
//	public List<BillingNoticeMain> findByStatusAndTemplatIds(List<String> status, List<String> tempIds);
	...
	@Transactional(rollbackFor = Exception.class, timeout = 30)
	void updatePnpMainEvery8dStatus(String status, Date modifyTime, Long pnpMainId);
