...
$(function(){
	...
	$('.btn_add').click(function(){
 		window.location.replace(bcs.bcsContextPath +'/edit/keywordResponseCreatePage?from=keywordResponseDisablePage');
	});
	...
	var loadDataFunc = function(){
		...
		$.ajax({
			type : "GET",
			url : bcs.bcsContextPath +'/edit/getInteractiveMsgList?type=KEYWORD&status=DISABLE'
		}).success(function(response){
			$('.dataTemplate').remove();
			...
			for(key in response.MsgMain){
				...
				msgData.find('.mainKeyword a').attr('href', bcs.bcsContextPath +'/edit/keywordResponseCreatePage?iMsgId=' + iMsgId + '&actionType=Edit&from=keywordResponseDisablePage');
