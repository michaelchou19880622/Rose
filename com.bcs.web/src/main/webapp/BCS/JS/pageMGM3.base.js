...
$(function(){
	$('.btn_add').click(function(){
 		window.location.replace(bcs.bcsContextPath + '/edit/shareCampaignCreatePage?actionType=Create&from=disable');
	});
	...
	var btn_copyFunc = function(){
		...
		console.info('btn_copyFunc campaignId:' + campaignId);
 		window.location.replace(bcs.bcsContextPath + '/edit/shareCampaignCreatePage?campaignId=' + campaignId + '&actionType=Copy&from=disable');
	};
	...
	var loadDataFunc = function(){
		...
		$.ajax({
			type : "GET",
			url : bcs.bcsContextPath + '/edit/getShareCampaignListDisable'
		}).success(function(response){
			$('.dataTemplate').remove();
			console.info(response);
...
//				msgContent += '<img src="' + bcs.bcsContextPath + '/getResource/IMAGE/' + o.campaignImageId + '" alt="Type2" style="cursor: pointer; width:100px"><br>';
				...
				queryBody.find('.campaignTitle a')
					.attr('href', bcs.bcsContextPath + '/edit/shareCampaignCreatePage?campaignId=' + o.campaignId + '&actionType=Edit&from=disable')
					.html(msgContent);
