...
import com.bcs.core.taishin.circle.PNP.scheduler.ExecuteSendPnpTask;
import com.bcs.core.taishin.circle.PNP.scheduler.LoadFtbPnpDataTask;
import com.bcs.core.taishin.circle.PNP.scheduler.SendPnpJob;
import com.bcs.core.utils.ErrorRecord;

@Service
public class CircleSchedulerService {
	private static final String MSG_SEND_GROUP = "MSG_SEND_GROUP";
	private static final String SCHEDULER_FLAG = "SCHEDULER_FLAG";
	private static final String PNP_SEND_GROUP = "PNP_SEND_GROUP";

	@Autowired
	private MsgMainService msgMainService;
	
	/** Logger */
	private static Logger logger = Logger.getLogger(CircleSchedulerService.class);
	...
	public CircleSchedulerService() throws Exception{
	...
	public void addLoadFtpPnpDataSchedule(String cronExpression) throws Exception {
        logger.debug("addLoadFtpPnpDataSchedule: 1");
        
        /**
         * Create JobDetail
         */
        JobDetail jobDetail = createJobDetailForLoadFtpPnpData("LoadFtbPnpDataTask", "LoadFtbPnpData");
        logger.debug(jobDetail);
        JobKey jobKey = jobDetail.getKey();
        ...
        try {
            synchronized (SCHEDULER_FLAG) {
                ...
                checkSchedule("addLoadFtpPnpDataSchedule", "1");

                Date result = scheduler.scheduleJob(jobDetail, trigger);

                logger.debug("addCommandSchedule result:" + result);
                logger.debug("addCommandSchedule detailName: LoadFtbPnpDataTask");
                onSchedulerList.put("LoadFtbPnpDataTask", jobKey);
            }
        } catch (SchedulerException e) { // Handle
            logger.error(ErrorRecord.recordError(e));
            throw new Exception("Schedule Error");
	...
	private JobDetail createJobDetailForLoadFtpPnpData(String detailName, String detailGroup) {
        JobDetailFactoryBean detailFactory = new JobDetailFactoryBean();
        detailFactory.setName(detailName);
        detailFactory.setGroup(detailGroup);
        detailFactory.setBeanName(detailName);

        detailFactory.setJobClass(LoadFtbPnpDataTask.class);
        /**
         * Create JobDetail
         */
        detailFactory.afterPropertiesSet();
	...
	public void addPnpSendSchedule(Long pnpMainId, Date startTime) throws Exception{
        logger.debug("addPnpSendSchedule:" + pnpMainId);

        String detailName = createPnpDetailName(pnpMainId);

        /**
         * Create PnpJobDetail
         */
        JobDetail jobDetail = createPnpJobDetail(pnpMainId);
        logger.debug(jobDetail);
       JobKey jobKey =  jobDetail.getKey();
       ...
       Trigger trigger = createSimpleTrigger(startTime, jobDetail);
        ...
        try {
            synchronized (SCHEDULER_FLAG) {
                ...
                checkSchedule("addPnpSendSchedule", pnpMainId);
	...
	private JobDetail createPnpJobDetail(Long pnpMainId){

        String detailName = createPnpDetailName(pnpMainId);
        String detailGroup = PNP_SEND_GROUP; 
        ...
        detailFactory.setJobClass(SendPnpJob.class);
        
        Map<String, Object> jobDataAsMap = new HashMap<String, Object>();
        ExecuteSendPnpTask runTask = new ExecuteSendPnpTask();
        jobDataAsMap.put("sendPnpTask", runTask);
        jobDataAsMap.put("pnpMainId", pnpMainId);
        detailFactory.setJobDataAsMap(jobDataAsMap);
	...
	private String createPnpDetailName(Long pnpMainId){
        return "PnpMainId:"+ pnpMainId;
    }
	
	public boolean deletePnpSendSchedule(Long pnpMainId) throws Exception{
        logger.info("deletePnpSendSchedule:" + pnpMainId);

        String detailName = createPnpDetailName(pnpMainId);

        if(pnpMainId != null){
            logger.info("pnpMainId : " + pnpMainId);

            synchronized (SCHEDULER_FLAG) {

                checkSchedule("deletePnpSendSchedule", pnpMainId);
                
                JobKey jobKey = onSchedulerList.get(detailName);
                if(jobKey != null){
                    boolean result = scheduler.deleteJob(jobKey);
    
                    logger.info("deletePnpSendSchedule Success pnpMainId:" + pnpMainId);
                    onSchedulerList.remove(detailName);
