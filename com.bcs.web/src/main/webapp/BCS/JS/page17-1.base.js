...
$(function(){
	$('.btn_add').click(function(){
 		window.location.replace(bcs.bcsContextPath +'/edit/blackKeywordResponseCreatePage?from=blackKeywordResponseDisablePage');
	});
	...
	var btn_copyFunc = function(){
		...
		console.info('btn_copyFunc iMsgId:' + iMsgId);
 		window.location.replace(bcs.bcsContextPath + '/edit/blackKeywordResponseCreatePage?iMsgId=' + iMsgId + '&actionType=Copy&from=blackKeywordResponseDisablePage');
	};
	...
	var loadDataFunc = function(){
		...
		$.ajax({
			type : "GET",
			url : bcs.bcsContextPath +'/edit/getBlackInteractiveMsgList?type=BLACK_KEYWORD&status=DISABLE'
		}).success(function(response){
			$('.dataTemplate').remove();
			...
			for(key in response.MsgMain){
				...
				msgData.find('.mainKeyword a').attr('href', bcs.bcsContextPath +'/edit/blackKeywordResponseCreatePage?iMsgId=' + iMsgId + '&actionType=Edit&from=blackKeywordResponseDisablePage');
