$(function() {
      ...
      .success(function(response) {
        ...
        if (postData.actionType === 'SaveDraft') {
          ...
          window.location.replace(
            bcs.bcsContextPath + '/edit/msgListSendedPage'
          );
        } else {
          alert('傳送成功');
        }
  ...
  var getSendMsgTime = function(sendingMsgType, isShowAlert) {
    ...
    if (sendingMsgType === 'DELAY') {
      ...
      sendingMsgTime +=
        ' ' + selectHour + ':' + selectMinuteOne + selectMinuteTwo + ':00';
      console.info('sendingMsgTime', sendingMsgTime);
    }
  ...
  $('.SendGroupQueryBtn').click(function() {
    ...
    $.ajax({
      ...
      url:
        bcs.bcsContextPath +
        '/market/getSendGroupQueryResult?groupId=' +
        sendGroupId
    })
      .success(function(response) {
        console.info(response);
  ...
  $('.SendPreviewBtn').click(function(event) {
    ...
    $.each(MsgFrameContents, function(index, value) {
      ...
      switch (value.detailType) {
        ...
        case 'LINK': // 連結訊息
          if (
            detailContent.linkUriParams.lastIndexOf('http://', 0) !== 0 &&
  ...
  $('.btn_cancel').click(function() {
    ...
    var url = '/edit/msgListPage';

    if (fromParam === 'msgListDraftPage') {
      url = '/edit/msgListDraftPage';
    } else if (fromParam === 'msgListDelayPage') {
      url = '/edit/msgListDelayPage';
    } else if (fromParam === 'msgListSendedPage') {
      url = '/edit/msgListSendedPage';
    } else if (fromParam === 'msgListSchedulePage') {
      url = '/edit/msgListSchedulePage';
    }
    window.location.replace(bcs.bcsContextPath + url);
  });
  ...
  $('[name="schedule"]').click(function() {
    ...
    $('#scheduleSelect .selectHour')
      .closest('.option')
      .css('display', '');
    $('#scheduleSelect .selectMinuteOne')
      .closest('.option')
      .css('display', '');
    $('#scheduleSelect .selectMinuteTwo')
      .closest('.option')
      .css('display', '');
  });
    ...
    $.ajax({
      ...
      .success(function(response) {
        ...
        $.each(SerialSettingList, function(i, o) {
          var option = $(
            '<option value="' + o.serialId + '">' + o.serialTitle + '</option>'
          );
    ...
    if (msgId || msgSendId) {
      ...
      $.ajax({
        ...
        .success(function(response) {
          ...
          // 回寫 資料
          for (var key in response.MsgMain) {
            ...
            // 回寫 發送時間
            $('[name="sendTimeType"][value="' + keyObj.sendType + '"]').prop(
              'checked',
              true
            );
            ...
            if ('DELAY' === keyObj.sendType) {
              ...
              if (scheduleTime) {
                ...
                if (splits[1]) {
                  $('#delaySelect .selectMinuteOne').val(
                    splits[1].split(':')[1].substr(0, 1)
                  );
                }
                $('#delaySelect .selectMinuteOne').change();

                if (splits[1]) {
                  $('#delaySelect .selectMinuteTwo').val(
                    splits[1].split(':')[1].substr(1, 2)
                  );
                }
            ...
            // 回寫 發送時間
              ...
              if (scheduleTime) {
                var splits = scheduleTime.split(' ');
                console.info('splits', splits);
                $('[name="schedule"][value="' + splits[0] + '"]').prop(
                  'checked',
                  true
                );
                ...
                $('#scheduleSelect .selectMinuteOne').val(
                  timeStr.split(':')[1].substr(0, 1)
                );
                $('#scheduleSelect .selectMinuteOne').change();

                $('#scheduleSelect .selectMinuteTwo').val(
                  timeStr.split(':')[1].substr(1, 2)
                );
