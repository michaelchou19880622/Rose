...
public interface ContentLinkTracingRepository extends EntityRepository<ContentLinkTracing, Long>{
	...
	@Query(value = "SELECT * FROM (" +
			       ...
			       "OR (SELECT LINK_TITLE FROM BCS_CONTENT_LINK bcl WHERE bcl.LINK_ID = bclt.LINK_ID) LIKE ?3 " +
			       "OR (SELECT LINK_TITLE FROM BCS_CONTENT_LINK bcl WHERE bcl.LINK_ID = bclt.LINK_ID_BINDED) LIKE ?3 " +
			       "OR (SELECT LINK_TITLE FROM BCS_CONTENT_LINK bcl WHERE bcl.LINK_ID = bclt.LINK_ID_UNMOBILE) LIKE ?3 " +
			       ") AS NewTable " +
