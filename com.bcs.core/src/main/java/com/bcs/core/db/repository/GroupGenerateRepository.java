...
public class GroupGenerateRepository {
    ...
    @SuppressWarnings("unchecked")
    public String checkMIDBySendGroupDetailGroupIdForKeywordResponse(Long groupId, String mid) throws Exception {
        List<SendGroupDetail> sendGroupDetails = sendGroupDetailRepository.findBySendGroupGroupId(groupId);
        Query query = buildFindQueryForKeywordResponse(sendGroupDetails, "MID", mid);
        List<String> result = query.getResultList();
        if (result != null && !result.isEmpty()) {
            return result.get(0);
        } else {
     ...
     * @throws Exception
     */
    private Query buildFindQueryForKeywordResponse(List<SendGroupDetail> sendGroupDetails, String selectColumns, String mid) throws Exception {
        Validate.notEmpty(sendGroupDetails);
        Validate.notEmpty(selectColumns);
        ...
        log.info("UploadMid Size: {}, SendGroupSetting Size: {}", uploadMidSetting.size(), sendGroupSetting.size());
        if (!sendGroupDetails.isEmpty()) {
            sb.append(generateMidFieldSettingFromForKeywordResponse(sendGroupDetails, 1));
        }
        ...
        if (!uploadMidSetting.isEmpty()) {
            if (!sendGroupDetails.isEmpty()) {
                sb.append(", ");
                sb.append(generateUploadMidSettingFromForKeywordResponse(uploadMidSetting, sendGroupDetails.size() * 2 + 1));
            } else {
                sb = new StringBuilder();
                sb.append(String.format(" SELECT %s FROM", selectColumns));
                sb.append(generateUploadMidSettingFromForKeywordResponse(uploadMidSetting, 1));
                if (StringUtils.isNotBlank(mid)) {
                    sb.append(" WHERE MID = ?");
                    sb.append((uploadMidSetting.size() + 1));
                }
     ...
     * @return SQL String
     */
    private String generateMidFieldSettingFromForKeywordResponse(List<SendGroupDetail> sendGroupDetails, int params) {
        log.info("generateMidFieldSettingFrom start!!");
        if (sendGroupDetails == null || sendGroupDetails.isEmpty()) {
            return null;
        }
        ...
        SendGroupDetail detail = sendGroupDetails.get(0);
        sb.append(" WHERE f.KEY_DATA = ?" + params + " and f.VALUE " + detail.getQueryOp() + " ?" + (params + 1) + " ");
        sb.append(" AND u.status != 'BLOCK' and (u.STATUS = 'BINDED' OR u.STATUS = 'UNBIND' OR u.STATUS = 'SYSADD') ");
     ...
     * @return
     */
    private String generateUploadMidSettingFromForKeywordResponse(List<SendGroupDetail> sendGroupDetails, int params) {
        log.info("generateUploadMidSettingFrom Start!!");
        if (sendGroupDetails == null || sendGroupDetails.isEmpty()) {
            return null;
        }
        ...
        sqlString += " AND k.status != 'BLOCK' and (k.STATUS = 'BINDED' OR k.STATUS = 'UNBIND' OR k.STATUS = 'SYSADD') ";
