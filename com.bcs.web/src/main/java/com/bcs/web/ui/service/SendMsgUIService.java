...
public class SendMsgUIService {
    ...
    public void sendMessage(SendMsgModel sendMsgModel, String adminUserAccount) throws Exception {
        ...
        if (groupId == null) {
            ...
            throw new BcsNoticeException("未進行發送，參數錯誤，群組ID不可為空值");
        }

        SendGroup sendGroup = sendGroupService.findOne(groupId);
        if (sendGroup == null) {
            logger.error("sendGroup Null");
            throw new BcsNoticeException("未進行發送，因查無此發送群組");
        }

        switch (sendMsgModel.getSendingMsgType()) {
            case MsgMain.SENDING_MSG_TYPE_IMMEDIATE:
                immediateProcess(sendMsgModel, adminUserAccount, sendGroup);
                break;
            case MsgMain.SENDING_MSG_TYPE_DELAY:
                delayProcess(sendMsgModel, adminUserAccount);
                break;
            case MsgMain.SENDING_MSG_TYPE_SCHEDULE:
                scheduleProcess(sendMsgModel, adminUserAccount);
                break;
            default:
                throw new Exception("UnKnow SendingMsgType : " + sendMsgModel.getSendingMsgType());
        }
    }

    private void scheduleProcess(SendMsgModel sendMsgModel, String adminUserAccount) throws Exception {
    ...
    private void delayProcess(SendMsgModel sendMsgModel, String adminUserAccount) throws Exception {
    ...
    private void immediateProcess(SendMsgModel sendMsgModel, String adminUserAccount, SendGroup sendGroup) throws Exception {
        if (sendGroup.getGroupId() > 0) {
            // 行銷人員設定 群組
            try {
                List<String> midList = groupGenerateService.findMIDBySendGroupDetailGroupId(sendGroup.getGroupId());
                if (midList == null || midList.isEmpty()) {
                    throw new BcsNoticeException("未進行發送，因查無可發送用戶。");
                }
            } catch (Exception e) {
                logger.error("Exception", e);
                throw new BcsNoticeException("未進行發送，查詢發送清單失敗。");
            }
        } else {
            // 預設群祖
            Long result = sendGroupService.countDefaultGroupSize(sendGroup.getGroupId());
            if (result == null) {
                throw new Exception("未進行發送，查詢預設群組失敗。");
            }
            if (result <= 0) {
                throw new BcsNoticeException("未進行發送，因查無可發送用戶。");
            }
