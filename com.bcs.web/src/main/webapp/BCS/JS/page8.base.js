...
$(function(){
	$('.btn_add').click(function(){
 		window.location.href=bcs.bcsContextPath + '/edit/couponCreatePage?actionType=Create&from=active';
	});
	$('.ActiveBtn').click(function(){
 		window.location.href=bcs.bcsContextPath +'/edit/couponListPage';
	});
	$('.DisableBtn').click(function(){
 		window.location.href=bcs.bcsContextPath +'/edit/couponListDisablePage';
	});
	
	var btn_serialNumber = function(){
		var couponId = $(this).attr('couponId');
 		window.location.href=bcs.bcsContextPath +'/edit/couponSerialNumberPage?couponId='+couponId;
	};
	var btn_winnerList = function(){
		var couponId = $(this).attr('couponId');
 		window.location.href=bcs.bcsContextPath +'/edit/couponWinnerListPage?couponId='+couponId;
	};
	
	var btn_copyFunc = function(){
		var couponId = $(this).attr('couponId');
		console.info('btn_copyFunc couponId:' + couponId);
 		window.location.href=bcs.bcsContextPath + '/edit/couponCreatePage?couponId=' + couponId + '&actionType=Copy&from=active';
	};
	...
	var redesignFunc = function(){
		...
		if (!confirm('若優惠券有綁定活動，可能會影響使用者權益，請確認是否取消')) {
			return false;
		}
	...
	var loadDataFunc = function(){
		...
		$.ajax({
			type : "GET",
			url : bcs.bcsContextPath + '/edit/getContentCouponListAndEventReference'
		}).success(function(response){
			$('.dataTemplate').remove();
			console.info(response);
			...
			$.each(response, function(i, o){
				...
				queryBody.find('.couponTitle a')
					.attr('href', bcs.bcsContextPath + '/edit/couponCreatePage?couponId=' + contentCoupon.couponId + '&actionType=Edit&from=active')
					.html(msgContent);
				
				queryBody.find('.couponEventReference a')
					.html(contentCoupon.eventReference==='REWARD_CARD'?'集點卡:</br>'+eventReferenceTitle:contentCoupon.eventReference==='SCRATCH_CARD'?'刮刮樂:</br>'+eventReferenceTitle:'無')
					.attr('href', 
							contentCoupon.eventReference==='REWARD_CARD'?
									bcs.bcsContextPath +'/edit/rewardCardCreatePage?rewardCardId='+ contentCoupon.eventReferenceId +'&actionType=Edit&from=active':
										contentCoupon.eventReference==='SCRATCH_CARD'?
												bcs.bcsContextPath + '/edit/gameCreatePage/scratchCard?gameId='+ contentCoupon.eventReferenceId +'&actionType=Edit':'無'
						);
				...
				queryBody.find('.btn_serialNumber')
					.attr('couponId', contentCoupon.couponId)
					.click(btn_serialNumber);
				
				queryBody.find('.btn_winnerList')
					.attr('couponId', contentCoupon.couponId)
					.click(btn_winnerList);
