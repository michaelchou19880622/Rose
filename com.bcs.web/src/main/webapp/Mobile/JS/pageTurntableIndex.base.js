$(function (){
	...
	var initPage = function(){
		...
		$.ajax({
			...
			console.info("obtainedPrizeId :  ",response["obtainedPrizeId"][0]);
			obtainedPrizeId = response["obtainedPrizeId"][0];
			
			prizeCount = (gameData.length-5)/6;
			
			for(var i=0; i<prizeCount; i++){
				prizeList.push({
					prizeName : gameData[i*6+5],
					imgId : gameData[i*6+6],
					prizeContent : gameData[i*6+7],
					prizeId : gameData[i*6+10]					
				});
			}
	...
	var rotateFn = function (awards, angles, txt){
		...
		$('#rotate').rotate({
			...
			callback:function (){
				...
				prizeTr.find('.prizeImage img').attr('src', "../bcs/getCdnResource/IMAGE/" + prizeList[awards].imgId);
				prizeTr.find('.prizeName').html(prizeList[awards].prizeName);
				prizeTr.find('.prizeContent').html(prizeList[awards].prizeContent);
	...
	$('.pointer').click(function (){
		if(bRotate)return;
		var item = obtainedPrizeId;
		var angle = 360/prizeCount;
		
		for(var i=0; i<prizeCount; i++){
			if(prizeList[i].prizeId == obtainedPrizeId){
				rotateFn(i, i*angle, prizeList[i].prizeName);
				break;
			}
		}
	});
