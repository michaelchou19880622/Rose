...
public interface SystemLogRepository extends EntityRepository<SystemLog, Long>, SystemLogRepositoryCustom {
    ...
    Page<SystemLog> findByTargetAndAction(String target, String action, Pageable pageable);
    ...
    Long countByLevel(String level, String start, String end);
    ...
    Long countByTargetAndAction(String target, String action, String start, String end);

    @Transactional(readOnly = true, timeout = 30)
    @Query(value = "SELECT COUNT('x') FROM BCS_SYSTEM_LOG WHERE MODIFY_DAY >= ?1 AND MODIFY_DAY < ?2", nativeQuery = true)
    Long countAll(String start, String end);
    ...
    List<SystemLog> findByTargetAndAction(String target, String action, String start, String end);
