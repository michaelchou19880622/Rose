...
public interface PnpContentLinkRepository extends EntityRepository<PnpContentLink, String> {
    @Transactional(readOnly = true, timeout = 30)
    @Query(value = "SELECT LINK_URL, LINK_TITLE, LINK_ID, MODIFY_TIME "
            + "FROM BCS_PNP_CONTENT_LINK "
    ...
    @Transactional(readOnly = true, timeout = 30)
    @Query(value = "SELECT LINK_URL, LINK_TITLE, LINK_ID, MODIFY_TIME "
            + "FROM BCS_PNP_CONTENT_LINK, BCS_PNP_CONTENT_FLAG "
...
//	@Query("select x from ContentLink x where x.linkId in ?1 ") // MYSQL Difference
    ...
    @Query(value = "SELECT "
            ...
            + "FROM BCS_PNP_CONTENT_LINK, BCS_USER_TRACE_LOG "
