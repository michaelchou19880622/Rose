...
public class SendingPnpHandlerSend extends UntypedActor {
	...
	/** Logger */
	private static Logger logger = Logger.getLogger(SendingPnpHandlerSend.class);
	...
	public void onReceive(Object message) throws Exception {
		...
		if (message instanceof AsyncPnpSendModel) {
			...
			logger.debug("AsyncPnpSendModel onReceive:" + msgs);

			if(msgs.getPnpDetails() != null){
				
				logger.debug("Size:" + msgs.getPnpDetails().size());
				...
				for(PnpDetail detail : msgs.getPnpDetails()){
					try {
						...
						logger.error("phone:" + detail.getPhone());
...
//                        deliveryTags.add(detail.getPhoneHash());
			...
			logger.debug("AsyncPnpSendingModelError End");
		}
	}
