...
$(function(){
	...
	$('.btn_add').click(function(){
 		window.location.replace(bcs.bcsContextPath +'/edit/keywordResponseCreatePage?from=keywordResponseIneffectivePage');
	});
	...
	var loadDataFunc = function(){
		...
		$.ajax({
			type : "GET",
			url : bcs.bcsContextPath +'/edit/getInteractiveMsgList?type=KEYWORD&status=INEFFECTIVE'
		}).success(function(response){
			$('.dataTemplate').remove();
			...
			for(key in response.MsgMain){
				...
				msgData.find('.mainKeyword a').attr('href', bcs.bcsContextPath +'/edit/keywordResponseCreatePage?iMsgId=' + iMsgId + '&actionType=Edit&from=keywordResponseIneffectivePage');
				...
				if(keyObj.interactiveStatus == "取消"){
					$(this).find('.interactiveStatus input').val('生效');
				}else {
					$(this).find('.interactiveStatus input').val('取消');
				}
