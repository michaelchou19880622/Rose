...
$(function () {
    ...
    title.innerHTML = source.pageTitle;
    // ---- Global Variables ----
    // input data
    ...
    // get List Data
    var getListData = function (name, url, sourceType) {
        ...
        $.ajax({
            ...
            switch (name) {
            case '啟用':
                getListData('停用', '/pnpAdmin/getPNPMaintainAccountList?status=false', sourceType);
                break;
            case '停用':
                $('.LyMain').unblock();
                break;
    ...
    $('.btn_add.search').click(function () {
        ...
        $('.LyMain').block($.BCS.blockMsgRead);
        // get all list data
        getListData('啟用', '/pnpAdmin/getPNPMaintainAccountList?status=true', source.type);
    });
    ...
    $('#normalSearchBtn').click(function () {
        ...
        $('.LyMain').block($.BCS.blockMsgRead);
        getListData('啟用', '/pnpAdmin/getPNPMaintainAccountList?status=true', 'Normal');
    });

    $('#unicaSearchBtn').click(function () {
        if (templateCount > 0) {
            deleteTemplate();
            deleteTemplate();
        }
        $('.LyMain').block($.BCS.blockMsgRead);
        getListData('啟用', '/pnpAdmin/getPNPMaintainAccountList?status=true', 'Unica');
    });
