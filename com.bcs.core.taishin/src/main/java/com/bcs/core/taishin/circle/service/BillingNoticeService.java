...
public class BillingNoticeService {
	...
	public void updateMainAndDetailStatus(BillingNoticeMain billingNoticeMain, String status) {
		...
		billingNoticeMainRepository.updateBillingNoticeMainStatus(status,now,
				billingNoticeMain.getNoticeMainId());
	}
	...
	public void updateBillingNoticeMainStatusComplete(Long mainId) {
		...
		if (billingNoticeDetailRepository.countByNoticeMainIdAndStatus(mainId,
				status) == 0) {
			Date  now = Calendar.getInstance().getTime();
			billingNoticeMainRepository.updateBillingNoticeMainStatus(BillingNoticeMain.NOTICE_STATUS_COMPLETE,now, mainId);
		}
	}
	 ...
	 * @param selfActorRef
	 */
	public void pushLineMessage(BillingNoticeMain billingNoticeMain, ActorRef sendeRef, ActorRef selfActorRef) {
		...
		for (BillingNoticeDetail detail : details) {
			...
			try {
				...
				logger.info("NOTICE_STATUS_RETRY NoticeDetailId:" + detail.getNoticeDetailId());
				logger.error("NOTICE_STATUS_RETRY KeyManagementException | NoSuchAlgorithmException :" + e1.getMessage());
				detail.setStatus(BillingNoticeMain.NOTICE_STATUS_RETRY);
			} catch (Exception e) {
				logger.info("NOTICE_STATUS_RETRY NoticeDetailId:" + detail.getNoticeDetailId());
				logger.error("NOTICE_STATUS_RETRY Exception:" + e.getMessage());
				detail.setStatus(BillingNoticeMain.NOTICE_STATUS_RETRY);
