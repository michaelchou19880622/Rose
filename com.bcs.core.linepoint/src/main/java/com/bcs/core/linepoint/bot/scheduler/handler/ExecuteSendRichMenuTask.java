package com.bcs.core.linepoint.bot.scheduler.handler;
...
import com.bcs.core.linepoint.akka.service.LinePointSendAkkaService;
...
public class ExecuteSendRichMenuTask {
	...
	public void executeSendMsg(List<String> uids, String richMenuId) throws Exception{
		...
		LinePointSendAkkaService LinePointSendAkkaService = ApplicationContextProvider.getApplicationContext().getBean(LinePointSendAkkaService.class);
		
		try {
			LinePointSendModel LinePointSendModel = new LinePointSendModel();
			
			JSONArray uid = new JSONArray();
			for(String u : uids) {
				uid.put(u);
			}
			LinePointSendModel.setUid(uid);
			LinePointSendModel.setRichMenuId(richMenuId);
			LinePointSendAkkaService.tell(LinePointSendModel);
		}catch(Exception e){
			throw e;
		}
	}
