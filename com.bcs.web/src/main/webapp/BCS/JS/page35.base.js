...
$(function(){
	$('.btn_add').click(function(){
 		window.location.replace(bcs.bcsContextPath +'/edit/campaignResponseCreatePage?from=campaignResponsePage');
	});
	...
	var btn_copyFunc = function(){
		...
		console.info('btn_copyFunc iMsgId:' + iMsgId);
 		window.location.replace(bcs.bcsContextPath + '/edit/campaignResponseCreatePage?iMsgId=' + iMsgId + '&actionType=Copy&from=campaignResponsePage');
	};
	
	// 中獎名單按鈕
	var btn_winner_listFunc = function(){
		var iMsgId = $(this).attr('iMsgId');
		console.info('btn_winner_list iMsgId:' + iMsgId);
 		window.location.replace(bcs.bcsContextPath + '/edit/campaignUserListPage?iMsgId=' + iMsgId);
	};
	...
	var loadDataFunc = function(){
		...
		$.ajax({
			type : "GET",
			url : bcs.bcsContextPath +'/edit/getCampaignInteractiveMsgList?type=CAMPAIGN&status=ACTIVE'
		}).success(function(response){
			$('.dataTemplate').remove();
			...
			for(key in response.MsgMain){
				...
				msgData.find('.mainKeyword a').attr('href', bcs.bcsContextPath +'/edit/campaignResponseCreatePage?iMsgId=' + iMsgId + '&actionType=Edit&from=campaignResponsePage');
				...
				msgData.find('.btn_winner_list').attr('iMsgId', iMsgId);
				msgData.find('.btn_winner_list').click(btn_winner_listFunc);
