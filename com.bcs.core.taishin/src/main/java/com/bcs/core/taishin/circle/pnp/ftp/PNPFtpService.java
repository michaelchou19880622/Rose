...
public class PNPFtpService {
    ...
    private boolean loginFtp(FTPClient pFtpClient, PnpFtpSetting setting) {
        try {
            ...
            // T 以系統預設密碼登入
            if (!lStatus) {
                final String defId = "ACCOUNT";
                final String defPw = "PASSWORD";
                lStatus = pFtpClient.login(defId, defPw);
                if (lStatus) {
    ...
    private Session loginSFtp(PnpFtpSetting setting) {
        ...
        try {
            ...
            if (StringUtils.isNotBlank(account) && StringUtils.isNotBlank(password)) {
                ...
                if (lStatus) {
                    log.info(String.format("Login SFTP: %s 資源密碼系統的帳號密碼登入完成", setting.getHost()));
                } else {
    ...
    private Session smsLoginSFTP(PnpFtpSetting setting) {
        ...
        try {
            ...
            // T 先以資源密碼系統取得的帳號密碼進行登入，登入失敗再以系統原本設定的帳號密碼登入系統
            final String account = setting.getSmsAccount();
            final String password = setting.getSmsPassword();
            if ((account != null && !account.trim().isEmpty()) && (password != null && !password.trim().isEmpty())) {
            ...
            if (!lStatus) {
                ...
                session = null;
                // T 重新取得FTP 參數設定值
                final String host = setting.getSmsHost();
                final String serverHostName = setting.getSmsServerHostName();
                final int serverHostNamePort = setting.getSmsServerHostNamePort();
                final String appCode = setting.getSmsAPPCode();
                final String resCode = setting.getSmsRESCode();                
                
                final Map<String, String> trendPwMgmt = loadFtp(host, serverHostName, serverHostNamePort, appCode, resCode);
                log.info(String.format("SMS Login SFTP: %s PWD: %s", trendPwMgmt.get("uid"), trendPwMgmt.get("pwd")));

                session = jsch.getSession(trendPwMgmt.get("uid"), setting.getSmsHost(), setting.getSmsPort());
                session.setPassword(trendPwMgmt.get("pwd"));
                session.setConfig(sshConfig);
                session.connect();
                lStatus = session.isConnected();
                ...
                if (!lStatus) {
                    ...
                    lStatus = session.isConnected();
                    if (lStatus) {
                        log.info("smsLoginSFTP:" + setting.getSmsHost() + "原系統記錄帳號密碼登入完成");
                    } else {
                        log.error("smsLoginSFTP:" + setting.getSmsHost() + "原系統記錄帳號密碼登入失敗，停止執行");
                    }
                }
            }
        } catch (Exception ex) {
    ...
    private Map<String, byte[]> downloadMultipleFileInFTPForDev(String pDirectory, String extension, PnpFtpSetting setting) {
         ...
         *  UNICA來源   UNICA = "4"
         */
        final String source = setting.getChannelId();
     ...
     * @return 下載的檔案內容Map
     * @see LoadFtpPnpDataTask#parseDataFlow(PnpFtpSourceEnum)
                ...
                if (!source.equals(AbstractPnpMainEntity.SOURCE_EVERY8D) && !source.equals(AbstractPnpMainEntity.SOURCE_UNICA)) {
                    ...
                    for (FTPFile file : lFiles) {
                        ...
                        if (!file.isDirectory() && fileName.toUpperCase().endsWith("TXT")) {
                            ...
                            log.info("fileName :" + fileName);
                            log.info("ftpClient.printWorkingDirectory() :" + ftpClient.printWorkingDirectory());
                            log.info("fileNewName: {}", ftpClient.rename(fileName, fileName + ".ok"));
                        }
                    }
                    FTPFile[] filesOk = ftpClient.listFiles();
                    for (FTPFile file : filesOk) {
    ...
    private Map<String, byte[]> downloadMultipleFileInSFTP(String pDirectory, String extension, PnpFtpSetting setting) {
        ...
        try {
            ...
            if (channelSftp.isConnected()) {
                ...
                if (!source.equals(AbstractPnpMainEntity.SOURCE_EVERY8D) && !source.equals(AbstractPnpMainEntity.SOURCE_UNICA)) {
                    //三竹、明宣需要使用rename依現行簡訊平台檢核機制 若可以rename為.ok表示檔案上傳完畢即可抓走，若無法rename表示檔案正在上傳  >>by 志豪 20190422 mail【台新 Line PNP】相關問題
                    Vector<ChannelSftp.LsEntry> list = channelSftp.ls("*." + extension.toLowerCase());
                    list.addAll(channelSftp.ls("*." + extension.toUpperCase()));
                    for (ChannelSftp.LsEntry lFtpFile : list) {
                        log.info("sftp start rename!!!!");
                        final String fileName = lFtpFile.getFilename();
                        log.info("downloadMultipleFileInSFTPForDev fileName:" + fileName);
                        channelSftp.rename(fileName, fileName + ".ok");
                    }

                    Vector<ChannelSftp.LsEntry> listOk = channelSftp.ls("*." + extension.toLowerCase() + ".ok");
                    listOk.addAll(channelSftp.ls("*." + extension.toUpperCase() + ".ok"));
                    for (ChannelSftp.LsEntry lFtpFile : list) {
                        final String fileName = lFtpFile.getFilename();
                        log.info("downloadMultipleFileInSFTPForDev fileName:" + fileName);
                        ByteArrayOutputStream lDataTemp = new ByteArrayOutputStream();
...
//				}
    ...
    private void uploadFileInFTP(Map<String, InputStream> targetStreamMap, String targetDir, PnpFtpSetting setting) throws IOException {
        ...
        try {
			...
			/*                
            //lambda may be memory leak for ftpClient local variable.   
    ...
    private void uploadFileInSFTP(Map<String, InputStream> targetStreamMap, String targetDir, PnpFtpSetting setting) {
        ...
        try {
            ...
            try {
                ...
                if (content == null) {
                    final boolean mkdirIsSuccess = mkdir(targetDir, channelSftp);
    ...
    private boolean mkdir(String path, ChannelSftp sftpChannel) {
        try {
            ...
            if (isNotExist(path, sftpChannel)) {
                final String[] arr = path.substring(1).split("/");
