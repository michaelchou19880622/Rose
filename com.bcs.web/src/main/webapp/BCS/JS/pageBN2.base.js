...
$(function(){
	...
	var loadDataFunc = function(){
		...
		$.ajax({
			type : "GET",
			url : bcs.bcsContextPath + '/edit/getBillingNoticeBigSwitch'
		...
		// get all list data
		getListData('全部', '/edit/getBillingNoticeAllList');
//		getListData('生效', '/edit/getBillingNoticeOnList');
//		getListData('取消', '/edit/getBillingNoticeOffList');
	...
	var getListData = function(name, url){
		$.ajax({
			...
			for(key in response){
				...
				templateMsgTr.find('.templateMsgImgTitle a').attr('href', bcs.bcsContextPath + '/edit/billingNoticeCreatePage?templateId=' + key + '&actionType=Edit');				
	...
	$('#toCreatePage').click(function(){
 		window.location.replace('billingNoticeCreatePage');
	});

	// to Edit Page
	var templateMsgSelectEventFunc = function(){
		var templateId = $(this).attr('templateId');
 		window.location.replace(bcs.bcsContextPath + '/edit/billingNoticeCreatePage?templateId=' + templateId + '&actionType=Edit');
	};
	...
	$('.btn_copy').click(function(e) {
		...
		window.location.replace(bcs.bcsContextPath + '/edit/billingNoticeCreatePage?templateId=' + selectedTemplateId + '&actionType=Copy');
	});
	...
	$('.btn_detele').click(function(e) {
		...
		$.ajax({
			type : "DELETE",
			url : bcs.bcsContextPath + '/admin/deleteBillingNotice/' + selectedTemplateId
		}).success(function(response){
			alert("刪除成功！");
	...
	$('#btnNoticeSwitch').click(function(){
		...
		$.ajax({
			type : "GET",
			url : bcs.bcsContextPath + '/edit/setBillingNoticeBigSwitch/' + noticeSwitch
		}).success(function(response){
			alert("變更成功！");
			window.location.replace(bcs.bcsContextPath + '/edit/billingNoticeListPage');
		}).fail(function(response){
